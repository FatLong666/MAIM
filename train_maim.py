...
from maim.data.dataset import LAMPDataset
from maim.pipelines.pipeline_maim import MAIMPipeline
from maim.util import save_videos_grid, ddim_inversion
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    for name, module in unet.named_modules():
        # with open('model_analysis/model_names.txt', 'a') as file:
        #     file.write(f"{name}\n")
        if name.endswith(tuple(trainable_modules)):

            for params in module.parameters():
                params.requires_grad = True
        # if name.endswith(tuple('attn_img_temp')):
        #     for params in module.parameters():
        #         print('attn_img_temp:', params.requires_grad)
        #         with open('model_analysis/att_img_temp_1.txt', 'a') as file:
        #             file.write(f"{params.requires_grad}\n")
    ...
    train_dataset = LAMPDataset(**train_data, image_encoder=image_encoder, image_processor=image_processor)

    # Preprocessing the dataset
    for p in train_dataset.prompt:
        train_dataset.prompt_ids.append(
            tokenizer(p, 
    ...
    validation_pipeline = MAIMPipeline(
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                img_feats = batch['img_features'].to(weight_dtype)
                img_feats = rearrange(img_feats, "b f h w -> (b f) h w")
                ...
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states, img_feats).sample
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % validation_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if validation_data.use_inv_latent == False:
                        ...
                        for idx, prompt in enumerate(validation_data.prompts):
                            ...
                            # validation_img_feat = image_encoder(image_processor(image, return_tensors="pt")['pixel_values'].to('cuda'))['last_hidden_state']
                            # validation_img_feat = repeat(validation_img_feat, "b c h -> (b f) c h", f=video_length)

                            # timestep = torch.randint(0, noise_scheduler.num_train_timesteps, (1,), device='cuda')
                            # timestep = timesteps.long()
                            # noise = torch.randn_like(first_frame_latents)
                            # noisy_first_frame_latents = noise_scheduler.add_noise(first_frame_latents, noise, timestep)
    ...
    if accelerator.is_main_process:
        ...
        pipeline = MAIMPipeline.from_pretrained(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/flower-blooms.yaml")
