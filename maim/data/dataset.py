...
class LAMPDataset(Dataset):
    def __init__(
        ...
        # 新增image_encoder和image_processor
    ...
    def __getitem__(self, index):
        ...
        # ===TODO===
        # 利用clip提取image_features
        img_feats = torch.stack([self.image_encoder(
            self.image_processor(video[i], return_tensors="pt")['pixel_values'].to('cuda'))[
                                     'last_hidden_state'].squeeze(0) for i in range(len(video))], dim=0)
        # ===TODO===
